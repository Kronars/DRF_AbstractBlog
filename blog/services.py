...
def validate_search_params(title: str = False, name_cat: str = False, pub_date: str = False,
    ...
    if pub_date:
        ...
        if pub_datetime > datetime.now(): raise Http404('Я дико извиняюсь, а ты как в будущем статьи искать собрался?')
    ...
    if desc:
        if order:   # Если выбрана сортировка по столбцу, символ - меняет порядок сортировки на возрастающий
